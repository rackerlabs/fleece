...
positional arguments:
  {import,export,edit,render,upload}
    ...
    render              Render configuration for a stage
    upload              Upload configuration to SSM
...
optional arguments:
  ...
  --environments ENVIRONMENTS, -e ENVIRONMENTS
                        Path to YAML config file with defined accounts and
                        stage names. Defaults to ./environments.yml
```
...
- `--environments` an environments.yml file that defines the different environments (or stages) and the associated AWS account for each. The format is as described in the `fleece run` command.

The config commands work with two types of config files. The `config.yml` file is a "closed" config file, where all sensitive values are encrypted. Developers typically do not edit this file but instead export it to an "open" configuration file where sensitive variables appear in plain text for editing. As soon as changes are made, the open config file is imported, which re-encrypts everything and writes an updated `config.yml`.
...
#### `fleece config render [--json] <stage>`

Writes the configuration variables for the given stage to stdout. YAML and JSON formats are supported.

#### `fleece config upload [--erase] --path PATH <stage>`

Uploads the configuration to the EC2 Parameter Store, under the given path, with nested dictionaries and lists flattened. Encrypted variables are written as `SecureString`, while other variables are written are regular `String` values. If the `--erase` option is given, old values under the given path are erased, but new values are not uploaded in their place.

The `fleece.parameters.read_parameters(path)` function reads the parameters from the parameter store and returns a restored configuration dictionary that preserves the original hierarchy and structure.